...
PclTestCore::PclTestCore(ros::NodeHandle &nh){
    ...
    nh.param<float>("the_view_number", view_number, 1);
    //水平视场角，单位度
...
//视场限制
void PclTestCore::limit(const pcl::PointCloud<pcl::PointXYZ>::Ptr in,
...
//区域裁剪
void PclTestCore::clip(const pcl::PointCloud<pcl::PointXYZ>::Ptr in,
                       const pcl::PointCloud<pcl::PointXYZ>::Ptr out)
{
    pcl::ExtractIndices<pcl::PointXYZ> clipper;//创建ExtractIndices对象
...
void PclTestCore::point_cb(const sensor_msgs::PointCloud2ConstPtr & in_cloud_ptr){
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr limited_pc_ptr(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PointCloud<pcl::PointXYZ>::Ptr clipped_pc_ptr(new pcl::PointCloud<pcl::PointXYZ>);
